...
# Output image name
OUTPUT_IMAGE_NAME="ubuntu-${UBUNTU_RELEASE}-preinstalled-${IMAGE_TYPE}-${ARCH}+raspi-updated.img"
...
BOOT_MOUNT_POINT="/mnt/pi_boot-${CODE_NAME}-${IMAGE_TYPE}"
ROOT_MOUNT_POINT="/mnt/pi_root-${CODE_NAME}-${IMAGE_TYPE}"
...
rm -f "$(join_paths "/tmp" "$OUTPUT_IMAGE_NAME")" # Only remove the final output image, keep the base image for re-runs
rm -f "$(join_paths "${TEMP_DIR}" "$COMPRESSED_OUTPUT_IMAGE_NAME")" # Also remove compressed output if it exists
